# Atributos y métodos
# Si hay algo que ilustre el potencial de la POO esa es la capacidad de definir variables 
# y funciones dentro de las clases, aunque aquí se conocen como atributos y métodos respectivamente.
...
# Atributos dinámicos
# Dado que Python es muy flexible los atributos pueden manejarse de distintas formas, por
# ejemplo se pueden crear dinámicamente(al vuelo) en los objetos.
...
# tener que definir los atributos de esa forma es tedioso. 
# Es más práctico definir unos atributos básicos en la clase. 
# De esa manera todas las galletas podrían tener unos atributos por defecto:
class Galleta:
    chocolate = False
...
# las siguientes galletas se crearán con chocolate, es decir, 
# habremos modificado las instrucciones de creación de los objetos:
class Galleta:
    chocolate = False
...
# Métodos
# Si por un lado tenemos las "variables" de las clases, 
# por otro tenemos sus "funciones", 
# que evidentemente nos permiten definir funcionalidades para llamarlas desde las instancias.
# Definir un método es bastante simple, sólo tenemos que añadirlo en la clase 
# y luego llamarlo desde el objeto con los paréntesis, como si de una función se tratase:
class Galleta:
    chocolate = False
...
# Sin embargo, al intentar ejecutar el código anterior desde una 
# galleta veréis que no funciona. Nos indica que el método saludar() 
# requiere 0 argumentos pero se está pasando uno.
# ¿Cómo puede ser? Si en ningún momento hemos enviado ninguna información a al galleta...
# Lo que tenemos aquí, estimados alumnos, es la diferencia fundamental entre métodos de clase 
...
# ¿por qué al llamarlo desde el objeto dice que estamos enviando un argumento?
...
# ¿Curioso que haya funcionado verdad? Además ¿no os suena de algo ese resultado que muestra 
# el parámetro que hemos definido? Se trata de la propia representación del objeto.
...
# <__main__.Galleta object at 0x0000028E654769E8 >
# Pues sí, podemos acceder al propio objeto desde el interior de sus métodos. 
# Lo único que como este argumento hace referencia al objeto en sí mismo por convención se le llama self.
# Poder acceder al propio objeto desde un método es muy útil, 
# ya que nos permite acceder a sus atributos. Fijaros, el siguiente código no funcionaría como esperamos:
...
# tienen que saber quienes son porque sino no pueden acceder sus atributos internos 
# y por eso tienen que enviarse asimismas a los métodos.
# Sea como sea con este ejemplo podemos entender que por defecto el valor de un atributo 
# se busca en la clase, pero para modificarlo en la instancia es necesario hacer referencia al objeto.

# Métodos especiales
# Ahora que sabemos crear métodos y hemos aprendido para qué sirve el argumento self, 
# es momento de introducir algunos métodos especiales de las clases.
...
# permitiéndonos definir valores nulos, valores por posición y nombre, argumentos indeterminadas, etc.

# Destructor
# Si existe un constructor también debe existir un destructor 
# que se llame al eliminar el objeto para que encargue de las tareas de limpieza como vaciar la memoria. 
# Ese es el papel del método especial del. 
...
# y es que pese a que tienen accesores en forma de función para facilitar su llamada, es totalmente posible ejecutarlos directamente como si fueran métodos normales:
...
# como str() y len(), y es que en efecto, esas también son accesores de los métodos especiales 
# str y len que tienen los objetos.
...
# Y sí, digo definirlo y no redefinirlo porque por defecto no existe en 
# los objetos aunque sea el que se ejecuta al pasarlos a la función len().
